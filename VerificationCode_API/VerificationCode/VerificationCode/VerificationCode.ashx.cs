...
namespace SmallTools.VerificationCode
{
    /// <summary>
    ...
    public static class SetSql
    {
        public static string Add(string code)
        {
           string sql = string.Format(@"INSERT INTO dbo.VerificationCode (Code,token) VALUES ('{0}','{1}')select @@identity", code, Randoms.GetRandomString.GetString(20));
           return DBHelper.SqlBase.SearchData(sql).Rows[0][0].ToString();
        }

        public static void AddErrorCount(string Id)
        {
            string sql = string.Format("UPDATE VerificationCode SET code_errornum = code_errornum + 1 WHERE Id = '{0}' ", Id);
            DBHelper.SqlBase.UpdateData(sql);
        }
	...
	code_errornum,
token
FROM dbo.VerificationCode where ID = '{0}' And DataState = 0 ",ID);
           DataTable dt = DBHelper.SqlBase.SearchData(sql);
        ...
        public static void Delete(string ID,string isCheck)
        {
            string sql = string.Format("UPDATE VerificationCode SET DataState = 1,isCheck = '{1}' WHERE ID = {0} ", ID,isCheck);
            DBHelper.SqlBase.UpdateData(sql);
        }
    }
